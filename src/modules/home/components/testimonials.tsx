...
const TestimonialsSection = () => {
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          // For left images: find by .fade-in-left then add animate-fade-in-left
          document.querySelectorAll<HTMLImageElement>(".fade-in-left").forEach((img) => {
          ...
          // For right images: find by .fade-in-right then add animate-fade-in-right
          document.querySelectorAll<HTMLImageElement>(".fade-in-right").forEach((img) => {
            const duration = `${Math.random() * 2 + 1}s`;
            const delay = `${Math.random() * 2}s`;
            img.style.animationDuration = duration;
            img.style.animationDelay = delay;
  ...
  return (
    <section ref={sectionRef} className="flex items-center justify-between w-full">
      <div className="flex items-center gap-x-2.5">
        <div className="flex flex-col gap-y-2.5">
          <img 
            className="fade-in-left opacity-0"
            src={kundenBild1.src} 
            alt="Customer 1" 
      ...
      <div className="flex flex-col gap-y-16 items-center">
        <div className="flex flex-col gap-y-5 items-center">
          <Stars/>
          <h2 className="text-7xl text-center font-title">
            Das beliebteste Hubbett von<br/>
            <span ref={wordRef}>{words[currentWord]}</span>
          </h2>
        </div>
        <Quotes className="absolute translate-y-44"/>
        <div className="max-w-4xl overflow-hidden flex flex-col items-center"
          ...
          <Splide
            ...
            className="w-full px-20 py-5"
          >
            {testimonials.map((text, index) => (
              <SplideSlide key={index}>
                <div className="testimonial-slide flex items-center justify-center h-full w-full">
      ...
      <div className="flex items-center gap-x-2.5">
        <div className="flex flex-col gap-y-2.5 items-end">
          <img 
            className="fade-in-right opacity-0"
            src={kundenBild6.src} 
            alt="Customer 6" 
